...
export default function Layout() {
  ...
  return <div className="app-shell"><RouteSeo/><PageAnalytics/><ClientErrorReporter/><a className="skip-link" href="#main-content">Skip to content</a>
    ...
    <footer className="footer"><div className="footer-grid container-wide"><div className="footer-about"><span className="brand footer-brand">VELOURA<span>WOMEN</span></span><p>A women-only fashion destination for everyday style, occasion dressing, accessories and beauty.</p><Link className="footer-story-link" to="/help/about">Our story →</Link></div><div><h2>Online shopping</h2><Link to="/shop?category=womens-dresses">Dresses</Link><Link to="/shop?category=womens-tops">Tops</Link><Link to="/shop?category=womens-ethnicwear">Ethnic wear</Link><Link to="/shop?category=womens-shoes">Footwear</Link><Link to="/shop?category=womens-beauty">Beauty</Link></div><div><h2>Customer policies</h2><Link to="/help/shipping">Shipping</Link><Link to="/help/returns">Returns & refunds</Link><Link to="/help/size-guide">Size guide</Link><Link to="/help/privacy">Privacy</Link><Link to="/help/terms">Terms</Link></div><div><h2>Useful links</h2><Link to="/track-order">Track order</Link><Link to="/orders">Orders</Link><Link to="/wishlist">Wishlist</Link><Link to="/help/contact">Contact us</Link><Link to="/help/faq">FAQ</Link></div><div className="footer-newsletter"><h2>Get the good stuff first</h2><p>New drops, big offers and editor picks—straight to your inbox.</p><form className="newsletter" onSubmit={(e) => e.preventDefault()}><input type="email" placeholder="Email address" required /><button>Join</button></form><small>By joining, you agree to receive Veloura marketing emails.</small></div></div><div className="footer-assurance container-wide"><div><strong>CURATED WOMEN'S STORE</strong><span>Multi-source catalog with quality gates</span></div><div><strong>EASY RETURNS</strong><span>30-day return window on eligible items</span></div><div><strong>PRIVATE ACCOUNT SYNC</strong><span>RLS-scoped data · payment remains sandboxed</span></div></div><div className="footer-bottom container-wide"><span>© 2026 Veloura Women</span><span>Fashion, beauty and accessories — curated for women</span></div></footer>
  </div>
}
