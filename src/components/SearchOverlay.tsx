...
export default function SearchOverlay({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const productMatches = useMemo(() => normalized ? catalog.filter((product) => [product.title, product.brand, product.category, ...(product.tags ?? [])].filter(Boolean).join(' ').toLowerCase().includes(normalized)).slice(0, 7) : [], [catalog, normalized])
  const categoryMatches = useMemo(() => normalized ? WOMEN_CATEGORIES.filter((category) => `${category.label} ${category.blurb}`.toLowerCase().includes(normalized)).slice(0, 5) : [], [normalized])
  ...
  return <div className="search-overlay" onMouseDown={onClose}>
    <section className="search-overlay-panel" onMouseDown={(event) => event.stopPropagation()}>
      <div className="search-overlay-head"><form onSubmit={submit}><Search size={21}/><input autoFocus value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search dresses, brands, beauty, bags and more"/><button type="submit">Search</button></form><button className="search-overlay-close" onClick={onClose}><X size={20}/></button></div>
      {!normalized ? <div className="search-start-grid">
        ...
        <div className="search-popular-categories"><h3>Popular departments</h3>{WOMEN_CATEGORIES.slice(0,8).map((category) => <button key={category.value} onClick={() => { navigate(`/shop?category=${category.value}`); onClose() }}><img src={category.image} alt=""/><span><strong>{category.label}</strong><small>{category.blurb}</small></span></button>)}</div>
      </div> : <div className="search-results-grid">
        <div className="search-results-main"><div className="search-result-title"><span>Products</span>{productMatches.length > 0 && <button onClick={() => go(query)}>View all results <ArrowRight size={14}/></button>}</div>{productMatches.length ? productMatches.map((product) => { const price = getProductPricing(product); return <button className="search-product-result" key={product.id} onClick={() => { navigate(`/product/${product.id}?category=${product.category}`); onClose() }}><img src={product.thumbnail} alt=""/><span><b>{product.brand || 'Veloura Edit'}</b><strong>{product.title}</strong><small>{formatINR(price.selling)}</small></span></button> }) : <div className="search-empty-mini">No instant product matches. Press Enter to search the wider marketplace.</div>}</div>
        <aside>{categoryMatches.length > 0 && <div><h3>Departments</h3>{categoryMatches.map((category) => <button key={category.value} onClick={() => { navigate(`/shop?category=${category.value}`); onClose() }}>{category.label}<ArrowRight size={13}/></button>)}</div>}{brandMatches.length > 0 && <div><h3>Brands</h3>{brandMatches.map(([brand,count]) => <button key={brand} onClick={() => { navigate(`/brand/${encodeURIComponent(brand)}`); onClose() }}>{brand}<small>{count} styles</small></button>)}</div>}</aside>
      </div>}
    </section>
  </div>
}
