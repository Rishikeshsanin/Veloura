...
function normalize(item: LooseObject, originalQuery: string): Product | null {
  ...
  const text = `${title} ${pickString(item,['category','type'])} ${originalQuery}`
  ...
  return {
    id: stableNumericId(840000,slug),
    title,
    description:`Women’s ${category.replace('womens-','').replaceAll('-',' ')} style discovered for “${originalQuery}”.`,
    category,
    price,
...
export async function fetchSearchExpansion(query: string) {
  ...
  if (/saree|sari|banarasi|ethnic|silk/i.test(query)) {
    const ethnic = await fetchVaanzariEthnic().catch(() => [])
    products = [...ethnic,...products]
  }
