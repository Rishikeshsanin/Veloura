import { ArrowLeftRight, ChevronLeft, ChevronRight, Heart, ImageOff, MapPin, Maximize2, Minus, Plus, ShieldCheck, Star, Truck, X } from 'lucide-react'
...
export default function ProductPage() {
  ...
  const { addToCart, toggleWishlist, isWishlisted, toggleCompare, isCompared, recordRecentlyViewed, recentlyViewed, preferenceSignals } = useShop()
  ...
  const deliveryEstimate = deliveryChecked ? estimateDelivery(pincode) : null
  ...
  return <>
    ...
    <div className="container-wide product-page">
      ...
      <div className="product-detail">
        ...
        <section className="product-info">{product.brand ? <Link className="eyebrow pdp-brand-link" to={`/brand/${encodeURIComponent(product.brand)}`}>{product.brand} · BRAND STORE</Link> : <span className="eyebrow">VELOURA EDIT</span>}<h1>{product.title}</h1>{product.rating !== undefined && <div className="detail-rating"><span><Star size={15} fill="currentColor"/> {product.rating.toFixed(1)}</span><b>{product.reviews?.length ? `${product.reviews.length} written reviews` : 'Catalog rating'}</b></div>}<p className="detail-description">{product.description}</p><div className="detail-price"><strong>{formatINR(pricing.selling)}</strong>{pricing.discount > 0 && <><s>{formatINR(pricing.mrp)}</s><span>({pricing.discount}% OFF)</span></>}</div><small className="tax-note">Catalog price shown · sandbox checkout does not create a real charge or tax transaction</small>{product.color && <div className="pdp-color-line"><i style={{backgroundColor:colorSwatch(product.color)}}/><span>Colour</span><strong>{product.color}</strong></div>}{colorways.length > 0 && <div className="pdp-colorways"><div className="detail-label"><strong>OTHER COLOURS</strong><span>{colorways.length} similar colourways</span></div><div>{colorways.map((item) => <Link key={item.id} to={`/product/${item.id}?category=${encodeURIComponent(item.category)}`} aria-label={`${item.color || 'Colour'}: ${item.title}`}><i style={{backgroundColor:colorSwatch(item.color)}}/><span>{item.color}</span></Link>)}</div></div>}

          <div className="detail-section" id="size-guide"><div className="detail-label"><strong>SIZE OPTIONS</strong><button onClick={() => setSizeGuideOpen(true)}>SIZE GUIDE</button></div>{availableSizes.length > 0 ? <div className="detail-sizes">{availableSizes.map((s) => <button className={s === size ? 'active' : ''} key={s} onClick={() => setSize(s)}>{s}</button>)}</div> : <p className="size-source-note">This catalog source did not supply selectable size options. The bag records the size as “Not specified”.</p>}</div>
          <div className="buy-row"><div className="quantity"><button onClick={() => setQty(Math.max(1,qty-1))}><Minus size={15}/></button><span>{qty}</span><button onClick={() => setQty(qty+1)}><Plus size={15}/></button></div><button className="button primary add-bag" disabled={soldOut} onClick={() => !soldOut && addToCart(product,size,qty)}>{soldOut ? 'Sold out' : 'Add to bag'}</button><button className={`button wishlist-detail ${wished ? 'active' : ''}`} onClick={() => toggleWishlist(product)}><Heart size={18} fill={wished ? 'currentColor' : 'none'}/>{wished ? 'Saved' : 'Wishlist'}</button><button className={`button compare-detail ${compared ? 'active' : ''}`} onClick={() => toggleCompare(product)}><ArrowLeftRight size={17}/>{compared ? 'Comparing' : 'Compare'}</button></div>

          <div className={`delivery-box ${soldOut ? 'sold-out-delivery' : ''}`}><h3>{soldOut ? 'Currently unavailable' : 'Delivery options'}</h3><div className="pincode"><MapPin size={18}/><input value={pincode} onChange={(event) => { setPincode(event.target.value.replace(/\D/g,'').slice(0,6)); setDeliveryChecked(false) }} placeholder="Enter 6-digit pincode" inputMode="numeric"/><button disabled={!canCheckDelivery} onClick={() => setDeliveryChecked(true)}>CHECK</button></div>{deliveryEstimate && <div className="delivery-result"><strong>Sandbox estimate · {formatDeliveryDate(deliveryEstimate.from)}–{formatDeliveryDate(deliveryEstimate.to)}</strong><span>{deliveryEstimate.zone} · {deliveryEstimate.days[0]}–{deliveryEstimate.days[1]} business days. This is a planning estimate only; no courier promise is made until fulfilment is activated.</span></div>}<p><Truck size={17}/> Sandbox delivery model: free above ₹1,499</p><p><ShieldCheck size={17}/> Represented 30-day return policy for eligible items; no physical return is created yet</p></div>
          <div className="product-confidence"><div><ShieldCheck size={17}/><span><strong>{confidence.label}</strong><small>Live catalog information</small></span></div><p>Images, pricing and available attributes come from the current catalog sources. Missing ratings, stock counts, colours, sizes or review counts stay unavailable rather than being filled with synthetic values.</p></div>
          <div className="offer-box"><h3>Sandbox checkout offers</h3><p><b>WELCOME OFFER</b> — Extra 10% off with code <strong>HELLOVELOURA</strong></p><p><b>ORDER OFFER</b> — ₹300 off on orders above ₹1,999</p></div>
          <details open><summary>Product details</summary><p>{product.description}</p>{product.color && <p><strong>Colour:</strong> {product.color}</p>}{product.occasion && <p><strong>Occasion:</strong> {product.occasion}</p>}</details><details><summary>Fit & sizing</summary><p>{product.sizes?.length ? `Source-listed sizes: ${product.sizes.join(', ')}. Use the Veloura size guide as a general reference; brand-specific sizing may vary.` : 'The catalog source did not supply selectable size options. The size guide is only a general reference.'}</p></details><details><summary>Material & care</summary><p>Follow the product care label where provided. For delicate finishes, prefer gentle washing, low heat and careful storage.</p></details><details><summary>Shipping & returns</summary><p>The sandbox checkout model represents free delivery above ₹1,499 and a 30-day return window for eligible items. No courier shipment or physical return is created while fulfilment is inactive.</p><Link to="/help/returns">Read represented return policy →</Link></details><ProductQuestions product={product}/>
          <VerifiedReviews productId={product.id}/>
          {product.reviews?.length ? <><ReviewSummary reviews={product.reviews}/><div className="reviews-block"><div className="detail-label"><strong>CATALOG REVIEWS</strong><span>{product.reviews.length} total</span></div>{product.reviews.slice(0,3).map((review,index) => <article key={`${review.reviewerEmail || review.reviewerName || 'review'}-${index}`}><span><Star size={12} fill="currentColor"/> {review.rating.toFixed(1)}</span><p>{review.comment}</p><small>{review.reviewerName || 'Catalog reviewer'}{formatReviewDate(review.date)}</small></article>)}</div></> : <div className="reviews-empty"><strong>Reviews</strong><p>No written reviews are available for this catalog item yet.</p></div>}
    ...
    <div className="mobile-pdp-buy"><div><small>{product.brand || 'Veloura selection'}</small><strong>{formatINR(pricing.selling)}</strong></div><button className="button primary" disabled={soldOut} onClick={() => !soldOut && addToCart(product,size,qty)}>{soldOut ? 'Sold out' : 'Add to bag'}</button></div>
