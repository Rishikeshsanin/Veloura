...
export default function HomePage() {
  ...
  useEffect(() => {
    fetchHomeCatalog().then((catalog) => { setProducts(catalog); setLoading(false) }).catch(() => setLoading(false))
  }, [])
  ...
  return <>
    ...
    {loading ? <LoadingRail eyebrow="THE MARKDOWN EDIT" title="Biggest source-listed deals" /> : merchandising.bestDeals.length > 0 && <DeferredProductRail eyebrow="THE MARKDOWN EDIT" title="Biggest source-listed deals" subtitle="Discounts shown only where the catalog source supplies enough pricing data." products={merchandising.bestDeals} href="/shop?sort=discount" />}
...
function LoadingRail({ eyebrow = 'SOURCE-RATED', title = 'Top rated', compact = false }: { eyebrow?: string; title?: string; compact?: boolean }) {
